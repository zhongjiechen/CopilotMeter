...
- **v0.1.19** — **Popover height is now capped at ~80% of the screen** so opening the menu bar on a small display (or expanding the *Remote hosts* panel) no longer pushes content off-screen. Anything past the cap is reachable via a native vertical scroll. Also hardened `↥ push skills` to pre-create the remote parent dir (`mkdir -p` via `--rsync-path`), so a brand-new remote without `~/.copilot/` works on the first click.
- **v0.1.18** — **Push skills + agents to remotes.** Each row in the *Remote hosts* panel now has a tiny `↥ push skills` button that one-way `rsync`s your local `~/.copilot/skills` and `~/.copilot/agents` to that host (uses `--delete`, so the remote becomes an exact mirror). Works whether the host is enabled for usage sync or not, runs off the main thread, surfaces file/byte counts in the tooltip, and is no-op when local dirs don't exist. Reuses the same `BatchMode=yes` SSH path as data sync — if remote pull works, push works.
- **v0.1.17** — **Critical fix:** the v0.1.16 AIU back-fill rescan inadvertently inserted duplicate `session.shutdown` rows (SQLite treats NULL `message_id` values as distinct in `UNIQUE` constraints, so the dedup constraint silently failed). Some sessions ended up with 100+ duplicate rows, multiplying their AIU contribution by the same factor. This release adds a one-shot migration that collapses each (session, model, host) group to a single canonical row (keeping the version with the largest authoritative AIU), and installs a partial unique index so re-ingestion is genuinely idempotent going forward. Verified math: token-based fallback matches authoritative `totalNanoAiu` exactly to within rounding (cache_read at 10% of input, cache_write at 125% for Anthropic). ([#22](https://github.com/zhongjiechen/CopilotMeter/pull/22))
