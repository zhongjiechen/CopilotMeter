...
- **v0.1.31** — **Count in-progress (always-on) session usage into today.** AI Credits are only written to disk at `session.shutdown`, so a Copilot CLI session kept open for days (e.g. in tmux) accrued messages all day but showed ~0 credits for today until it next shut down. The aggregator now suppresses the per-message estimate only up to a session/model's last authoritative shutdown; messages after it (today's in-progress work) are estimated and attributed to their own day. The estimate is calibrated from the session's own historical AI-Credits-per-output-token ratio (sanity-bounded) instead of an output-only token price, so the magnitude is realistic rather than a ~6× undercount. Historical days fully covered by shutdowns are unchanged, and there is no double counting once a real shutdown arrives.

- **v0.1.30** — **Retire the unreliable Cloud Agent heuristic; classify session-state usage as CLI.** Sessions in a machine's `~/.copilot/session-state` were run by a `copilot` process on that machine, so they are terminal CLI usage (or VS Code agent if registered in the VS Code chat DB). The old `hostType=github` → Cloud Agent rule only reflected an agent-worktree git context, not cloud dispatch, and caused large terminal-driven sessions to be mislabeled Cloud Agent and to flip-flop between syncs. Classification is now stable and time-independent: VS Code agent (sticky) or CLI. A one-time migration reclassifies existing Cloud Agent rows to CLI. Genuinely cloud-dispatched agent sessions never write to local session-state, so the Cloud Agent label is reserved for a future explicit cloud-origin import path.
